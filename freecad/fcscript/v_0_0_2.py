...
def dir_of_2points(p1, p2):
    return (p2 - p1).normalize()


def get_macros_dir():
    Standard, UserDefined = False, True
    root = Path(App.getUserMacroDir(UserDefined))
    if not root.exists():
        root = Path(App.getUserMacroDir(Standard))
    return root
...
#  └────────────────────────────────────────────────────────────────────────────┘
#    [SECTION] [Wire] Free Wire in 3D
#  ┌────────────────────────────────────────────────────────────────────────────┐

class Wire3D:
    def __init__(self, origin=(0,0,0)):
        self.origin = to_vec(origin)
        self.pos = self.origin
        self.edges = []

    def add_bspline(self, poles):
        vec_poles = [to_vec(p) for p in poles]
        bspline = Part.BSplineCurve([self.pos, *vec_poles])
        self.edges.append(Part.Edge(bspline))
        self.pos = vec_poles[-1]

    def add_segment(self, point):
        point = to_vec(point)
        if self.pos != point:
            segment = Part.Edge(Part.LineSegment(self.pos, point))
            self.edges.append(segment)
            self.pos = point
    
    def add_round_corner(self, point, radius, tangency=0.618033):
        point = to_vec(point)
        tangency = clamp(tangency, 0.2, 0.8)
        point_dist = radius * tangency
        point_dist2 = radius - point_dist
        tangent1 = self.tangent * point_dist
        p0 = self.pos
        p1 = self.pos + tangent1
        tangent1 = self.tangent * point_dist2
        p2 = p1 + tangent1
        v_tangent2 = dir_of_2points(p2, point) 
        tangent2 = v_tangent2 * point_dist2
        p3 = p2 + tangent2
        tangent2 = v_tangent2 * point_dist
        p4 = p3 + tangent2
        self.add_bspline((p1, p3, p4))
    ...
    def set_to(self, name):
        obj = App.ActiveDocument.getObject(name)
        if obj:
            obj.Shape = self.build()
        else:
            Part.show(self.build(), name)

    @property
    def tangent(self):
        last_edge = self.edges[-1]
        return last_edge.Curve.tangent(last_edge.LastParameter)[0].normalize()
...
def qt_get_widget_path(widget, index):
...
def save_widget_state(widget, name):
    root = get_macros_dir()
    data = {}
    index = 0
    for child in widget.findChildren(QtGui.QWidget):
        if hasattr(child, 'value'):       
            ...
            try:     
                ...
                print(f"Ignoring value of {path}")
    with open(Path(root, f"fcscript_{name}.json"), 'w') as f:
        f.write(json.dumps(data))


def load_widget_state(widget, name):
    file = Path(get_macros_dir(), f"fcscript_{name}.json") 
    if file.exists():
                ...
                if hasattr(child, 'value'):       
                    ...
                    if path in data:
                        try:     
                            ...
                            print(f"Ignoring value of {path} because it was not found")
